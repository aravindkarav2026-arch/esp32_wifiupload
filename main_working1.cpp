...
#define OLED_SDA     6    // SH1106 I2C SDA
#define OLED_SCL     7    // SH1106 I2C SCL
...
// Initialize Display Driver for SH1106 (128x64)
...
void updateOLEDDisplay(int percent, const char* statusMsg) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    display.print("IP: Offline Mode");
  }

  display.display(); // Push buffer to physical OLED screen
}
...
void checkAndSendTankLevel() {
  ...
  if (percent == -1) {
    ...
    updateOLEDDisplay(-1, WiFi.status() == WL_CONNECTED ? "Connected" : "Offline");
    return;
  }
  ...
  updateOLEDDisplay(percent, statusStr);
...
void setup() {
  ...
  // 3. Initialize I2C Bus & SH1106 OLED Display
  ...
  display.println("ESP32-C6 Tank Monitor");
  ...
  // 4. Set Wi-Fi Mode and start non-blocking connection attempt
  WiFi.mode(WIFI_STA);
